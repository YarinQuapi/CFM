...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "POST") {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      if (file === undefined) {
        res.end(500).json({ ok: false, error: "No file selected" });
        return;
      }

      const dir: string = Array.isArray(fields.path)
        ? fields.path[0]
        : fields.path ?? "/";
      ...
      await pool.query(
        "INSERT INTO files (id, name, path, size, uploader, createdAt) VALUES (uuid(), ?, ?, ?, ?, now())",
        [file?.originalFilename, fields.path, size, fields.userId]
      );
